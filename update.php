...
$r++;
$s=$s+$sahi;
$q=mysqli_query($con,"UPDATE `history` SET `skor`=$s,`level`=$sn,`sahi`=$r, date= NOW()  WHERE  email = '$email' AND eid = '$eid'")or die('Error124');
...
$w++;
$s=$s-$wrong;
$q=mysqli_query($con,"UPDATE `history` SET `skor`=$s,`level`=$sn,`wrong`=$w, date=NOW() WHERE  email = '$email' AND eid = '$eid'")or die('Error147');
}
if($sn != $total)
{
$sn++;
...
else if( $_SESSION['key']!='musat7')
{
$q=mysqli_query($con,"SELECT skor FROM history WHERE eid='$eid' AND email='$email'" )or die('Error156');
while($row=mysqli_fetch_array($q) )
{
$s=$row['skor'];
}
...
$sun=$s+$sun;
$q=mysqli_query($con,"UPDATE `rank` SET `skor`=$sun ,time=NOW() WHERE email= '$email'")or die('Error174');
}
